...
def validActions(board, row, col, turn):
    piece = board[turn][row, col]
    ...
    if piece == scoring.get('p'):
        moves = []
        if row < 7 and board[WHITE][row+1,col] == 0:
            moves.append((row+1, col))
            if row == 1 and board[WHITE][row+2,col] == 0:
                moves.append((row+2, col))
        if row < 7 and col > 0 and board[WHITE][row+1, col-1] != 0:
            moves.append((row+1,col-1))
        if row < 7 and col < 7 and board[WHITE][row+1, col+1] != 0:
            moves.append((row+1,col+1))
        return moves
    ...
    if piece == scoring.get('P'):
        ...
        if row > 0 and col > 0 and board[BLACK][row - 1, col - 1] !=0 :
            moves.append((row-1, col-1))
        if row > 0 and col < 7 and board[BLACK][row - 1, col + 1] !=0 :
            moves.append(row-1, col+1)
        return moves
    ...
    if piece == scoring.get['r'] or piece == scoring.get['R']:
        ...
        for i in range(row - 1, -1, -1):
            if board[BLACK][i, col] == 0 and board[WHITE][i, col] == 0:
                moves.append((i, col))
            else:
                break
        ...
        for j in range(col - 1, -1, -1):
            if board[BLACK][row, j] == 0 and board[WHITE][row, j] == 0:
                moves.append((row, j))
            else:
                break
